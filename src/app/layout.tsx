  ...
  return (
    <html lang="en" suppressHydrationWarning className={`${dmSans.variable} ${jetbrainsMono.variable} ${ebGaramond.variable} ${playfairDisplay.variable}`}>
      ...
      <body className="min-h-screen bg-[var(--bg)] text-[var(--text-primary)] font-[family-name:var(--font-body)]">
        ...
        <script dangerouslySetInnerHTML={{ __html: `(function(){try{var p=location.pathname;if(p==="/demo"||p==="/venture"||p==="/"||p==="/invention"||p==="/onboarding"||p.startsWith("/onboarding"))return;var t=localStorage.getItem("theme");if(t==="dark")document.documentElement.classList.add("dark")}catch(e){}}())` }} />
