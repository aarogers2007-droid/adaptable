...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (user) {
      ...
      if (profile?.role === "instructor") {
        ...
        if (!classes?.length) {
          // Check co-admin access on invention classes
  ...
  // ── Login form (shared between admin/teacher and student) ──
  return (
    <main className="flex min-h-screen items-center justify-center px-4">
      <div className="w-full max-w-sm">
        <button
          onClick={() => { setActiveSection(null); setError(null); setEmail(""); setPassword(""); }}
          className="text-sm text-[var(--text-muted)] hover:text-[var(--text-primary)] mb-6"
        >
          &larr; Back
        </button>
        ...
        <div className="mt-6">
          ...
          <div className="relative mt-6">
            ...
            <div className="relative flex justify-center text-xs"><span className="bg-[var(--bg)] px-2 text-[var(--text-muted)]">or sign in with email</span></div>
          </div>
            ...
            <div>
              ...
              <div className="relative mt-1">
                <input
                  ...
                  className="block w-full rounded-lg border border-[var(--border-strong)] px-4 py-3 pr-11 text-sm outline-none transition-colors focus:border-[var(--primary)] focus:ring-2 focus:ring-[var(--primary)]/15"
