...
export async function sendParentalConsentEmail(params: ParentalConsentEmailParams): Promise<{ ok: boolean; error?: string }> {
  ...
  const pn = params.platformName ?? "Adaptable";
  const pe = params.privacyEmail ?? "${pe}";

  const subject = `A child wants to use ${pn} — your permission is needed`;
  const text = `Hi,

A child has signed up for ${pn}, an AI-powered platform where students design and plan small businesses. Because the child is under 13, we are required by US law (COPPA) to ask for your permission before they can use it.
...
If you APPROVE this child using ${pn}, click here:
${params.verificationUrl}

The link will expire in 7 days. You can revoke permission anytime by emailing ${pe}.
...
<body style="font-family: -apple-system, system-ui, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; color: #111; line-height: 1.55;">
  <h2 style="color: #111; font-size: 20px; margin: 0 0 16px;">A child wants to use ${pn}</h2>
  <p>A child has signed up for ${pn}, an AI-powered platform where students design and plan small businesses. Because the child is under 13, we are required by US law (COPPA) to ask for your permission before they can use it.</p>
  ...
  <div style="margin: 32px 0;">
    <a href="${params.verificationUrl}" style="display: inline-block; background: #16a34a; color: white; padding: 14px 28px; border-radius: 8px; text-decoration: none; font-weight: 600;">Approve and let them use ${pn}</a>
  </div>

  <p style="font-size: 13px; color: #6b7280;">Link expires in 7 days. To revoke permission anytime, email ${pe}.</p>
