/**
 * IKIGAI WIZARD EVALUATION HARNESS — v3
 *
 * Round 3 upgrades from v2:
 *   - Mirrors round-3 prompt: customer reality check, commit-don't-clarify,
 *     already-running detection, family-business detection, risky pivot,
 *     expanded forbidden-phrase list
 *   - Rubric fix: when niche=needs_clarification, specificity is excluded from
 *     the average (refusal-as-correct cannot be specific by definition)
 *   - New SUCCESS-GUARD bucket (5 obviously-coherent personas): any
 *     needs_clarification = automatic 1 on coherence (catches over-refusal drift)
 *   - k=3 runs for high-variance buckets (risky, prompt-injection,
 *     family-business, contradictory). k=2 for everything else.
 *
 * Usage:
 *   npx tsx scripts/eval-ikigai-v3.ts
 */
...
// k=3 for high-variance buckets, k=2 for the rest
function runsForBucket(b: Bucket): number {
  if (b === "risky-monetization" || b === "prompt-injection" || b === "family-business" || b === "contradictory") return 3;
  return 2;
}
...
const PERSONAS: Persona[] = [
  ...
  // ── SUCCESS-GUARD (5) — obviously coherent. Any needs_clarification = automatic fail. ──
  // Catches over-refusal drift introduced by the needs_clarification escape hatch.
  { id: "g01-clear-tutor", bucket: "success-guard", studentName: "Maya",
    passions: ["solving algebra problems", "teaching kids", "Khan Academy"],
    skills: ["I tutor my younger brother daily", "I explain things simply", "I'm in honors math"],
    needs: ["middle schoolers in my neighborhood struggle with pre-algebra"],
    monetization: "$15/hour or pay-per-session",
    notes: "Crystal-clear tutoring path. needs_clarification here = over-refusal bug." },
  { id: "g02-clear-bake", bucket: "success-guard", studentName: "Hannah",
    passions: ["sourdough", "decorating cookies", "food TikTok"],
    skills: ["I bake every weekend", "consistent rise on my loaves", "icing piping"],
    needs: ["my neighbors keep asking when I'll sell loaves"],
    monetization: "weekly pickup at my house",
    notes: "Crystal-clear baking path. Should commit." },
  { id: "g03-clear-walk", bucket: "success-guard", studentName: "Eli",
    passions: ["dogs", "running", "being outside"],
    skills: ["calm with big dogs", "I walk 5+ miles a day", "people on my block already trust me"],
    needs: ["three families on my street need dog walkers after school"],
    monetization: "weekly cash per walk",
    notes: "Real demand named. Customer literally identified. Should commit." },
  { id: "g04-clear-edit", bucket: "success-guard", studentName: "Jordan",
    passions: ["video editing", "skateboarding", "CapCut"],
    skills: ["I edit my friends' skate clips already", "transitions", "music sync"],
    needs: ["my skater friends want better edits for their TikToks"],
    monetization: "per-video pricing",
    notes: "Already serving customers. Should commit, not clarify." },
  { id: "g05-clear-sell", bucket: "success-guard", studentName: "Asha",
    passions: ["beaded earrings", "jewelry making", "Pinterest"],
    skills: ["I've made 30+ pairs", "I have all the supplies", "I sell some at school already"],
    needs: ["girls at my school want unique earrings under $10"],
    monetization: "$5-10 per pair, cash at school",
    notes: "Already running mini-business. Should commit and level up." },
];
...
- why_this_fits: 2-3 sentences explaining why THIS student is the right person to build this and not someone else. Write like a 25-year-old founder talking to a 15-year-old, not like a LinkedIn post. Name a SPECIFIC skill, tool, or experience from their inputs that gives them an unfair advantage — something concrete like "you already know FL Studio" or "you've done 47 sales on Depop," not vague like "you're passionate about this." If the student has secondary interests with a genuine transferable skill (e.g., drawing anime → steady hands for nail detail), explain that specific connection. If there is no natural cross-skill connection, go deeper within the chosen lane instead — identify a non-obvious market gap, timing advantage, or customer insight the student hasn't articulated yet. Never force a connection between unrelated skills. FORBIDDEN PHRASES (do not use ANY of these): "perfect storm," "secret weapon," "secret sauce," "have you considered," "what most people don't realize," "leverage," "unlock," "synergy," "cracked the code," "natural arbitrage," "your superpower."`;
  ...
  const youngGuard = p.age && p.age <= 12
    ? "THIS STUDENT IS 12 OR YOUNGER. Rule 11 (Young Student Guard) applies — zero startup capital, skill/time-based ideas only.\n\n"
    : "";
  const userMessage = `${youngGuard}The student's name is ${p.studentName}${p.age ? ` (age ${p.age})` : ""}. Based on their Ikigai:
...
- If the persona is "age-12" and the idea requires ANY startup capital (materials, equipment, inventory, supplies to purchase), score capital_required ≤ 2. For 12-year-olds, ideas must be purely skill/time-based with $0 upfront cost. Only score 5 if the student can start with nothing but their own time and existing household items.
...
async function runAll(): Promise<Result[]> {
  ...
  for (let i = 0; i < PERSONAS.length; i++) {
    const p = PERSONAS[i];
    const runs: Run[] = [];
    const k = runsForBucket(p.bucket);
    process.stdout.write(`[${i + 1}/${PERSONAS.length}] ${p.id} (${p.bucket}, k=${k}) `);
    for (let r = 0; r < k; r++) {
      ...
      // SUCCESS-GUARD: any clarification on an obviously-coherent persona is auto-fail
      if (p.bucket === "success-guard" && synth.niche === "needs_clarification") {
        const failScore: JudgeScore = {
          specificity: 1, coherence: 1, no_forced_hybrid: 5,
          capital_required: 3, customer_realistic: 3,
          insight_quality: 1, name_quality: 1, total: 15,
          reasoning: "AUTO-FAIL: success-guard persona received needs_clarification. This is over-refusal drift — the inputs were obviously specific enough to commit.",
          red_flags: ["over-refusal-drift"],
        };
        runs.push({ idea: synth, score: failScore });
        process.stdout.write("F");
        if (r < k - 1) process.stdout.write(",");
        continue;
      }
...
// Rubric fix: when niche=needs_clarification, specificity is excluded from the
// per-run total (refusal-as-correct cannot be specific by definition). Also exclude
// insight_quality since "asking a clarifying question" insight is structurally lower.
// Effective max becomes 25 (5 dims × 5) for clarification cases, which we then
// scale back to /35 for comparability.
function effectiveTotal(run: Run): number | undefined {
  if (!run.score || !run.idea) return run.score?.total;
  if (run.idea.niche === "needs_clarification") {
    // Use 5 dims (coherence, no_hybrid, capital, customer, name) scaled to /35
    const sub =
      run.score.coherence +
      run.score.no_forced_hybrid +
      run.score.capital_required +
      run.score.customer_realistic +
      run.score.name_quality;
    return (sub / 25) * 35;
  }
  return run.score.total;
}
...
function writeReport(results: Result[]) {
  ...
  lines.push(`Runs per persona: 2 (3 for high-variance buckets: risky, prompt-injection, family-business, contradictory)`);
  ...
  lines.push("");
  lines.push("| Bucket | Runs | Avg/35 (rubric-fixed) | v1 cmp /30 | Spec | Coh | NoHyb | Capital | Customer | Insight | Name | Clarify% |");
  lines.push("|---|---|---|---|---|---|---|---|---|---|---|---|");
  for (const [bucket, rs] of buckets) {
    ...
    // Use rubric-fixed total: needs_clarification runs use 5-dim subset scaled to /35
    const fixedAvg = allRuns.reduce((s, r) => s + (effectiveTotal(r) ?? r.score!.total), 0) / allRuns.length;
    const v130Equiv =
      allRuns.reduce(
        (s, r) =>
          s +
          (r.idea?.niche === "needs_clarification"
            ? // For clarification, use 5-dim subset scaled to /30
              ((r.score!.coherence +
                r.score!.no_forced_hybrid +
                r.score!.capital_required +
                r.score!.customer_realistic +
    ...
    const clarifyCount = allRuns.filter((r) => r.idea?.niche === "needs_clarification").length;
    const clarifyPct = ((clarifyCount / allRuns.length) * 100).toFixed(0);
    lines.push(
      `| ${bucket} | ${allRuns.length} | ${fixedAvg.toFixed(2)} | ${v130Equiv.toFixed(2)} | ${avg("specificity")} | ${avg("coherence")} | ${avg("no_forced_hybrid")} | ${avg("capital_required")} | ${avg("customer_realistic")} | ${avg("insight_quality")} | ${avg("name_quality")} | ${clarifyPct}% |`
    );
  }
  lines.push("");
  ...
  if (allRuns.length) {
    const overall35 = (allRuns.reduce((s, r) => s + (effectiveTotal(r) ?? r.score!.total), 0) / allRuns.length).toFixed(2);
    const overall30 = (
      allRuns.reduce(
        (s, r) =>
          s +
    ...
    lines.push(`**Overall avg (rubric-fixed): ${overall35}/35** (${allRuns.length} judged runs)`);
    lines.push("");
    lines.push(`**v1-comparable equivalent: ${overall30}/30** (vs v1 baseline 21.78/30, v2 25.75/30)`);
    lines.push("");
  }
  ...
  for (const r of results) {
    ...
    if (r.avgScore !== undefined) {
      lines.push(`**Avg across ${r.runs.length} runs: ${r.avgScore.toFixed(1)}/35**`);
      lines.push("");
    }
...
// ── Main ──
(async () => {
  const totalRuns = PERSONAS.reduce((s, p) => s + runsForBucket(p.bucket), 0);
  console.log(`Running Ikigai eval v3: ${PERSONAS.length} personas, ${totalRuns} total synthesis calls`);
